...
function showPosition(position, is_user_location = true) {
    ...
    primaryToken = 'pk.eyJ1IjoiaWx2aXRzIiwiYSI6ImNsZGViNGowNDBibDAzb3E3cWQ5OThrc20ifQ.0ZyvQ-0dE8CqherBbekJIA'
    backupKey = ''
    const options = {
        method: 'GET',
    ...
    if (position) {
        request(url, options)
            .then((geoLocation => previewWeather(geoLocation.features[0], is_user_location)))
    } else {
