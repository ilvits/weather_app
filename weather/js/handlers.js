...
function getCurrentLocation() {
    ...
    navigator.geolocation.getCurrentPosition((currentLocation) => {
        var crd = currentLocation.coords;
        ...
        fetch(url, options)
            ...
            .then(result => {
                console.log(result.suggestions[0].value)
                console.log(`
                Широта: ${crd.latitude}
                Долгота: ${crd.longitude}
                Плюс-минус ${crd.accuracy} метров.
                `);
                alert(result.suggestions[0].value);
            })
...
function searchCancel() {
    ...
    listOfLocations.classList.remove('-translate-y-14', 'scale-[0.96]', 'z-0')
...
function parseSuggestions(suggestions) {
    ...
    for (const [key, value] of Object.entries(rawList)) {
        if (value.data.fias_level === "65") {
            ...
            let text = `${name.replace(capText, `<span class="text-yellow">${capText}</span>`)}, <span class="text-white/50">${region}${(countryCode !== userCountry) ? ', ' + country : ''}</span>`
...
function hideWeatherPreview(slide) {
    newLocationPreviewEl.classList.add('opacity-0', 'translate-y-full', '-z-10');
    newLocationPreviewEl.classList.remove('z-[120]');
    setTimeout(() => {
...
async function previewWeather(location) {
    newLocationPreviewEl.classList.remove('-z-10', 'hidden', 'translate-y-full')
    newLocationPreviewEl.classList.add('z-[120]')
    const searchText = input.value
    const name = location.dataset.name;
    const name_translit = location.dataset.name_translit;
    const latitude = location.dataset.latitude;
    const longitude = location.dataset.longitude;
    const countryCode = location.dataset.countryCode
    const id = location.dataset.id;
    const region = location.dataset.region;
    ...
    fetch(url, options)
        ...
        .then(weatherData => {
                        ...
                        <!-- Start of Content -->
                            <div id="main_info-${id}" class="grid gap-4 
                                ...
                                text-white" data-hs-collapse="#weather-details-${name_translit}">
                                    ...
                                    <div id="expand-arrow-d-${id}" class="absolute -bottom-2 flex justify-center w-full">
            ...
            newLocationPreviewEl.classList.remove('opacity-0', 'translate-y-full')
            ...
            weatherDetailsToggle.onclick = () => {
                if (detailInfo.clientHeight > 0) {
                    ...
                    document.querySelector(`#expand-arrow-d-${id}`).children[0].classList.remove('rotate-[-20deg]')
                    document.querySelector(`#expand-arrow-d-${id}`).children[1].classList.remove('rotate-[20deg]')
                    document.querySelector(`#expand-arrow-d-${id}`).children[0].classList.add('rotate-[20deg]')
                    document.querySelector(`#expand-arrow-d-${id}`).children[1].classList.add('rotate-[-20deg]')
...
function addNewLocation() {
    // add to object
    const id = document.querySelector('#preview-window .preview-window').dataset.id
    const data = document.querySelector(`li[data-id="${id}"]`)
    const name = data.dataset.name;
    const name_translit = data.dataset.name_translit;
    const region = data.dataset.region;
    const country = data.dataset.country;
    const latitude = data.dataset.latitude;
    const longitude = data.dataset.longitude;
    ...
    locations = addToObject(locations, name_translit, {
        "name": name,
